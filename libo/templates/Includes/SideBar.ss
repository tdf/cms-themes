<% if Level(3) %><% if Menu(3) %>
  <ul id="ThirdNavigation"><% control Menu(3) %>
   <li class="$LinkingMode"><a href="$Link" title=<% sprintf(_t('Navigation.GOTOPAGE','"Go to the %s page"'),$Title.XML) %> class="$LinkingMode levela"><span>$MenuTitle.XML<% if LinkOrSection == section %> »<% end_if %></a></span><% if Children %>
