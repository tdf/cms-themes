<% if Level(3) %><% if Menu(3) %>
  <ul id="ThirdNavigation"><% control Menu(3) %>
   <li class="$LinkingMode"><a href="$Link" title=<% sprintf(_t('Navigation.GOTOPAGE','"Go to the %s page"'),$Title.XML) %> class="$LinkingMode levela"><span>$MenuTitle.XML<% if LinkingMode == current %> »</a></span>
    ...
    <% end_control %></ul><% else %></a></span><% end_if %>
   </li><% end_control %>
  </ul><% end_if %><% else %>
